...
public class CostFeedPlusActivity extends BaseActivity {
    @BindView(R.id.name)
    EditText name;
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                try {
                    ...
                    CostFeedPlusActivity.this.runOnUiThread(new Runnable() {
                        @Override
                        public void run() {
                            adapter = new ArrayAdapter<CharSequence>(CostFeedPlusActivity.this,android.R.layout.simple_spinner_item, finalMidS);
                            stock.setAdapter(adapter);
                            stock.setOnItemSelectedListener(new CostFeedPlusActivity.OnItemSelectedListenerImpl());
                        }
                    });
                }catch (Exception e){
                    e.printStackTrace();
                            ...
                            public void onResponse(@NonNull Call call, @NonNull Response response) throws IOException {
                                ...
                                new Thread(() -> {
                                    try {
                                        ...
                                        Toast.makeText(CostFeedPlusActivity.this, jsonObject1.getString("msg"), Toast.LENGTH_SHORT).show();
                                        if (jsonObject1.getString("msg").equals("添加成功")) {
                                            CostFeedPlusActivity.this.finish();
                                        }
                                        Looper.loop();
