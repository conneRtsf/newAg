...
public class FishSalesAccountingPlusActivity extends BaseActivity {
    @BindView(R.id.name)
    EditText name;
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                try {
                    ...
                    FishSalesAccountingPlusActivity.this.runOnUiThread(new Runnable() {
                        @Override
                        public void run() {
                            adapter = new ArrayAdapter<CharSequence>(FishSalesAccountingPlusActivity.this,android.R.layout.simple_spinner_item, finalMidS);
                            stock.setAdapter(adapter);
                            stock.setOnItemSelectedListener(new FishSalesAccountingPlusActivity.OnItemSelectedListenerImpl());
                        }
                    });
                }catch (Exception e){
                    e.printStackTrace();
                ...
                public void onClick(View view) {
                    if (city.equals("无")) {
                        FishSalesAccountingPlusActivity.this.runOnUiThread(new Runnable() {
                            @Override
                            public void run() {
                                Toast.makeText(FishSalesAccountingPlusActivity.this, "请选择库存", Toast.LENGTH_SHORT).show();
                            }
                        });
                    }else{
                        for (int i = 1; i < BS.length; i++) {
                                    ...
                                    public void onResponse(@NonNull Call call, @NonNull Response response) throws IOException {
                                        ...
                                        new Thread(() -> {
                                            try {
                                                ...
                                                Toast.makeText(FishSalesAccountingPlusActivity.this, jsonObject1.getString("msg"), Toast.LENGTH_SHORT).show();
                                                if (jsonObject1.getString("msg").equals("售卖成功")) {
                                                    FishSalesAccountingPlusActivity.this.finish();
                                                }
                                                Looper.loop();
