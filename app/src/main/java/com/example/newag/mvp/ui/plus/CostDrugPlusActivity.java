...
public class CostDrugPlusActivity extends BaseActivity {
    @BindView(R.id.name)
    EditText name;
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                try {
                    ...
                    CostDrugPlusActivity.this.runOnUiThread(new Runnable() {
                        @Override
                        public void run() {
                            adapter = new ArrayAdapter<CharSequence>(CostDrugPlusActivity.this,android.R.layout.simple_spinner_item, finalMidS);
                            stock.setAdapter(adapter);
                            stock.setOnItemSelectedListener(new CostDrugPlusActivity.OnItemSelectedListenerImpl());
                        }
                    });
                }catch (Exception e){
                    e.printStackTrace();
                            ...
                            public void onResponse(@NonNull Call call, @NonNull Response response) throws IOException {
                                ...
                                new Thread(() -> {
                                    try {
                                        ...
                                        Toast.makeText(CostDrugPlusActivity.this, jsonObject1.getString("msg"), Toast.LENGTH_SHORT).show();
                                        if (jsonObject1.getString("msg").equals("添加成功")) {
                                            CostDrugPlusActivity.this.finish();
                                        }
                                        Looper.loop();
