...
public class VegetableSalesAccountingPlusActivity extends BaseActivity {
    @BindView(R.id.name)
    EditText name;
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                try {
                    ...
                    VegetableSalesAccountingPlusActivity.this.runOnUiThread(new Runnable() {
                        @Override
                        public void run() {
                            adapter = new ArrayAdapter<CharSequence>(VegetableSalesAccountingPlusActivity.this,android.R.layout.simple_spinner_item, finalMidS);
                            stock.setAdapter(adapter);
                            stock.setOnItemSelectedListener(new VegetableSalesAccountingPlusActivity.OnItemSelectedListenerImpl());
                        }
                    });
                }catch (Exception e){
                    e.printStackTrace();
                ...
                public void onClick(View view) {
                    if (city.equals("无")) {
                        VegetableSalesAccountingPlusActivity.this.runOnUiThread(new Runnable() {
                            @Override
                            public void run() {
                                Toast.makeText(VegetableSalesAccountingPlusActivity.this, "请选择库存", Toast.LENGTH_SHORT).show();
                            }
                        });
                    }else{
                        for (int i = 1; i < BS.length; i++) {
                            if(city.equals(BS[i])){
                                ...
                                paramsMap.put("sales", String.valueOf(inventory.getText()));
                                    ...
                                    public void onResponse(@NonNull Call call, @NonNull Response response) throws IOException {
                                        ...
                                        new Thread(() -> {
                                            try {
                                                ...
                                                Toast.makeText(VegetableSalesAccountingPlusActivity.this, jsonObject1.getString("msg"), Toast.LENGTH_SHORT).show();
                                                if (jsonObject1.getString("msg").equals("售卖成功")) {
                                                    VegetableSalesAccountingPlusActivity.this.finish();
                                                }
                                                Looper.loop();
